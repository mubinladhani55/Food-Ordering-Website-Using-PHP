...
$restaurant = isset($_GET['restaurant']) ? $_GET['restaurant'] : 'TandoorHut';
...
function updateCategoryItemCount($categoryId) {
    global $conn;
    $sql = "UPDATE tandoor_category SET items_count = (
        SELECT COUNT(*) FROM tandoor_menu_item 
        WHERE category_id = ?
    ) WHERE id = ?";
...
function getMenuItems($categoryId = null) {
    ...
    if ($categoryId) {
        $sql = "SELECT m.*, c.name as category_name, c.id as category_id 
                FROM tandoor_menu_item m 
                JOIN tandoor_category c ON m.category_id = c.id 
                WHERE c.id = ?";
        $stmt = $conn->prepare($sql);
...
function getCategories() {
    ...
    $sql = "SELECT c.*, (
                SELECT COUNT(*) 
                FROM tandoor_menu_item 
                WHERE category_id = c.id
            ) as items_count 
            FROM tandoor_category c";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'add_item':
                $stmt = $conn->prepare("INSERT INTO tandoor_menu_item (name, description, price, category_id, spicy_level, image_path) VALUES (?, ?, ?, ?, ?, ?)");
            ...
            case 'edit_item':
                $stmt = $conn->prepare("UPDATE tandoor_menu_item SET name=?, description=?, price=?, category_id=?, spicy_level=?, image_path=? WHERE id=?");
            ...
            case 'delete_item':
                $stmt = $conn->prepare("DELETE FROM tandoor_menu_item WHERE id=?");
            ...
            case 'update_cart_item':
                ...
                $restaurant = "Tandoor Restaurant"; // Fixed restaurant name for Tandoor
...
<head>
    ...
    <link rel="stylesheet" href="CSS/tandoor_menu.css">
    <style>
        .back-button {
...
<body>
    ...
    <div class="header">
        ...
        <p>Authentic Indian Flavors, Fresh from the Tandoor</p>
    ...
    <div class="modal" id="cartModal">
        <div class="modal-content">
            ...
            <div class="cart-total" id="modalCartTotal">
                ...
                <form id="orderForm" action="tandoor_payment.php" method="POST">
    ...
    <script src="JS/tandoor_menu.js"></script>
</body>
</html>