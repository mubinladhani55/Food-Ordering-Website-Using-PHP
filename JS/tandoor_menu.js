...
const RESTAURANT_TYPE = 'Tandoor Restaurant';
...
function findMenuItemById(itemId) {
    ...
    const cartItem = cart.find(i => i.uniqueId === `tandoor_${id}`);
    if (cartItem) {
        return {
            id: cartItem.id,
            name: cartItem.name,
...
function displayMenuItems(items) {
    ...
    menuGrid.innerHTML = items.map(item => {
        ...
        const cartItemId = `tandoor_${itemId}`;
        const cartItem = cart.find(i => i.uniqueId === cartItemId);
            ...
            <div class="menu-item">
                ...
                <div class="item-details">
                    ...
                    <div class="quantity-control">
                        <button class="quantity-btn minus" onclick="updateCart('tandoor', ${itemId}, -1)" ${quantity === 0 ? 'disabled' : ''}>-</button>
                        <input type="text" class="quantity-input" value="${quantity}" readonly>
                        <button class="quantity-btn plus" onclick="updateCart('tandoor', ${itemId}, 1)">+</button>
                    </div>
                </div>
            </div>
        `;
