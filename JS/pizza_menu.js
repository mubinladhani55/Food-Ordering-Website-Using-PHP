...
const RESTAURANT_TYPE = 'Pizza Restaurant';
...
function setupCartModal() {
    ...
    const placeOrderButton = document.querySelector('.payment-button');
    if (placeOrderButton) {
        placeOrderButton.addEventListener('click', function() {
            ...
            cart = [];
            localStorage.removeItem(SHARED_CART_KEY);
...
function findMenuItemById(itemId) {
    ...
    const cartItem = cart.find(i => i.uniqueId === `pizza_${id}`);
    if (cartItem) {
        return {
            id: cartItem.id,
            name: cartItem.name,
...
function displayMenuItems(items) {
    ...
    menuGrid.innerHTML = items.map(item => {
        ...
        const cartItemId = `pizza_${itemId}`;
        const cartItem = cart.find(i => i.uniqueId === cartItemId);
        ...
        return `
            <div class="menu-item">
                ...
                <div class="item-details">
                    <h3>${item.name}</h3>
                    ${item.size ? `<div class="size">${displaySize(item.size)}</div>` : ''}
                    ${item.spice_level ? `<div class="spicy-level">${getSpiceLevel(item.spice_level)}</div>` : ''}
                    <p>${item.description}</p>
                    <div class="price">Rs${parseFloat(item.price).toFixed(2)}</div>
                    <div class="quantity-control">
                        <button class="quantity-btn minus" onclick="updateCart('pizza', ${itemId}, -1)" ${quantity === 0 ? 'disabled' : ''}>-</button>
                        <input type="text" class="quantity-input" value="${quantity}" readonly>
                        <button class="quantity-btn plus" onclick="updateCart('pizza', ${itemId}, 1)">+</button>
                    </div>
                </div>
            </div>
        `;
