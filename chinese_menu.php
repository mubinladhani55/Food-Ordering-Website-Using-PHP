...
$restaurant = isset($_GET['restaurant']) ? $_GET['restaurant'] : 'ChineseRestaurant';

// Map restaurant names to their display names and database values
$restaurantMap = [
    'WorkOnFire' => ['display' => 'Chinese Restaurant', 'db' => 'ChineseRestaurant'],
    'PizzaHub' => ['display' => 'Pizza Hub', 'db' => 'PizzaHub'],
    'TandoorHut' => ['display' => 'Tandoor Hut', 'db' => 'TandoorHut']
];

// Get the display name and database value for the current restaurant
$currentRestaurant = $restaurantMap[$restaurant] ?? ['display' => 'Restaurant', 'db' => 'ChineseRestaurant'];
$currentRestaurantName = $currentRestaurant['display'];
$restaurantDbValue = $currentRestaurant['db'];

// Function to update category item count
function updateCategoryItemCount($categoryId) {
    global $conn;
    $sql = "UPDATE chinese_category SET items_count = (
        SELECT COUNT(*) FROM chinese_menu_item 
        WHERE category_id = ?
    ) WHERE id = ?";
...
function getMenuItems($categoryId = null) {
    ...
    if ($categoryId) {
        $sql = "SELECT m.*, c.name as category_name, c.id as category_id 
                FROM chinese_menu_item m 
                JOIN chinese_category c ON m.category_id = c.id 
                WHERE c.id = ?";
        $stmt = $conn->prepare($sql);
...
function getCategories() {
    ...
    $sql = "SELECT c.*, (
                SELECT COUNT(*) 
                FROM chinese_menu_item 
                WHERE category_id = c.id
            ) as items_count 
            FROM chinese_category c";
...
function saveOrderItem($orderId, $restaurant, $itemId, $itemName, $quantity, $price) {
    ...
    error_log("saveOrderItem: order_id=$orderId, restaurant=$restaurant, item_id=$itemId, item_name=$itemName, quantity=$quantity, price=$price, user_id=" . ($userId ?? 'NULL'));
    ...
    if ($result->num_rows > 0) {
        ...
        if ($newQuantity <= 0) {
            ...
            if (!$deleteStmt) {
                error_log("Delete query prepare failed: " . $conn->error);
                return false;
            }
            ...
            // Update quantity
            $updateSql = "UPDATE restaurant_orders SET quantity = ?, price = ? WHERE id = ?";
            ...
            if (!$success) {
                error_log("Update query failed: " . $updateStmt->error);
            }
            return $success;
        }
    } else if ($quantity > 0) {
        // New item, insert with user ID and default status as 'pending'
        $insertSql = "INSERT INTO restaurant_orders 
                      (order_id, restaurant, item_id, item_name, quantity, price, user_id, status) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')";
        ...
        if (!$success) {
            error_log("Insert query failed: " . $insertStmt->error);
        }
        return $success;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'add_item':
                $stmt = $conn->prepare("INSERT INTO chinese_menu_item (name, description, price, category_id, spicy_level, image_path) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("ssdiis", $_POST['name'], $_POST['description'], $_POST['price'], $_POST['category_id'], $_POST['spicy_level'], $_POST['image_path']);
            ...
            case 'edit_item':
                $stmt = $conn->prepare("UPDATE chinese_menu_item SET name=?, description=?, price=?, category_id=?, spicy_level=?, image_path=? WHERE id=?");
                $stmt->bind_param("ssdissi", $_POST['name'], $_POST['description'], $_POST['price'], $_POST['category_id'], $_POST['spicy_level'], $_POST['image_path'], $_POST['item_id']);
            ...
            case 'delete_item':
                $stmt = $conn->prepare("DELETE FROM chinese_menu_item WHERE id=?");
            ...
            case 'update_cart_item':
                ...
                // Use the database restaurant value
                if (saveOrderItem($orderId, $restaurantDbValue, $itemId, $itemName, $quantity, $price)) {
                    $response['success'] = true;
                    ...
                    error_log("Failed to save order item: order_id=$orderId, item_id=$itemId, quantity=$quantity");
                }
                break;
        }
    }
...
<body>
    ...
    <div class="modal" id="cartModal">
        <div class="modal-content">
            ...
            <div class="cart-total" id="modalCartTotal">
                ...
                <form id="orderForm" action="chinese_payment.php" method="POST">
    ...
    <script>
        ...
        const restaurantDbValue = "<?php echo $restaurantDbValue; ?>";
    </script>
    <script src="JS/chinese_menu.js"></script>
</body>
</html>